...
@implementation GameStageTVC_OFF
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (energyint < 5) {
        ...
        [self.offlineManager setOperatingTable:@"gameInfo_table"];
        self.curMissionModel = (MissionModel *)self.dataSource[indexPath.row];
        
        [self.offlineManager setOperatingTable:@"gameOrders_table"];
        NSObject *obj = [self.offlineManager getObjectWithKey:((MissionModel *)self.dataSource[indexPath.row]).missionID];
        
        [self analyzeServiceData:obj];
...
//        [self.curAudioManager prepareForAudioPlayer:localPath];
        dispatch_async(dispatch_get_main_queue(), ^{
            MainGameVC_OFF *vc = [[MainGameVC_OFF alloc] initWithGameOrderFile:self.gameOrderFile AndMissionModel:self.curMissionModel];
